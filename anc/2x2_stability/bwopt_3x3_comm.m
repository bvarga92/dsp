...
P2=1;
P3=1;
sendPeriod=[10 15 20]; % ennyi mintankent kuldjuk at az egyutthatokat, es frissitjuk az adaptiv szuroket
N=1e3;        % szimulacio hossza
omega0=pi/10; % referenciajel diszkret korfrekvenciaja (0...pi)
C=1;          % referenciajel amplitudoja
L=200;        % adaptiv szurok hossza
mu=1e-10;     % batorsagi tenyezo
...
for ii=1:N;
    ...
    % update elokeszitese
        ...
        W(1,:)=W(1,:)+DW_1(1,:);            % sajat update vegrehajtasa
        % node2
        ...
        W(2,:)=W(2,:)+DW_2(2,:);            % sajat update vegrehajtasa
        % node3
...
for ii=1:3
    settling(ii)=find(abs(e(ii,:))>C*0.1,1,'last');
end
...
for ii=1:3
    ...
    hold on;
    plot([settling(ii), settling(ii)],ylim,'r');
    hold off;
end
