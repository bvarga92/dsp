...
T=0.4;            %szimulacios ido (s)
N_S=5;            %masodlagos utakat modellezo FIR szurok hossza
N_W=300;          %adaptiv szurok hossza
mu=2e-7;          %batorsagi tenyezo
updatePeriod=100; %ennyi mintankent kuldjuk at az egyutthatokat, es frissitjuk az adaptiv szuroket
commDelay=50;     %kommunikacios kesleltetes
...
DW_1=zeros(2,N_W);           %node1 altal (a sajat hibajele alapjan) elkoeszitett update
DW_2=zeros(2,N_W);           %node2 altal (a sajat hibajele alapjan) elkoeszitett update
updateCntr=0;
commDelayCntr=0;
for ii=1:length(t);
    ...
    %kommunikacios kesleltetes szimulacioja
        if commDelayCntr>0
            commDelayCntr=commDelayCntr+1;
        end
        if commDelayCntr>=commDelay+2
            commDelayCntr=0;
            W=W+DW;
            DW=zeros(2,N_W);
        end
